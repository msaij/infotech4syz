...
export default function ClientProfilePage() {
  ...
  // Assume first group is the company
  const company = user?.groups && user.groups.length > 0 ? user.groups[0] : "Unknown";
  // Assume role is the same as group for now (customize if you have roles)
  const role = user?.permissions && user.permissions.length > 0 ? user.permissions.join(", ") : "User";

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Implement profile update logic for client
    setIsEditing(false);
  };
